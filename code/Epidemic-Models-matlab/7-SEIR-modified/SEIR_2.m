...
for idx =1:length(T)-1
    ...
    I(idx+1)=I(idx)+delta*E(idx)-(mu+alpha+sita)*I(idx);%患病人数迭代
    R(idx+1)=R(idx)+mu*I(idx)+mu_2*Q(idx);%康复人数迭代 
    D(idx+1)=D(idx)+alpha*I(idx)+alpha_1*Q(idx);%死亡患者人数迭代
    Q(idx+1)=Q(idx)+sita*I(idx)-alpha_1*Q(idx)-mu_2*Q(idx);%隔离人数迭代
    sum_I(idx+1) = sum_I(idx) + delta*E(idx);%累计患病人数迭代
    Iq(idx+1) = I(idx)+Q(idx);%现存总患病人数迭代
end
save('output2.txt',"sum_I","Iq","D","R",'-ascii');%累计确诊\现存确诊\累计死亡\累计出院
plot(T,S,T,E,T,I,T,R,T,D,T,Q);
grid on;
xlabel('日期');
ylabel('人数');
legend('易感者','潜伏者','感染者','康复者','死亡者','隔离者');
title('第二次改进的SEIR模型');
